...
from data_aug import data_augmentation, advanced_data_augmentation


class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, patch_n=None, patch_size=None, transform=None, augment_mode='basic'):
    ...
    def __getitem__(self, idx):
        ...
        input_path = input_img if self.load_mode == 0 else None
        if self.load_mode == 0:
        ...
        if self.patch_size:
            ...
            slice_id = os.path.basename(input_path).replace('_input.npy', '') if input_path else str(idx)
            return (input_img, target_img, slice_id)


def get_patch(full_input_img, full_target_img, patch_n, patch_size, augment_mode='basic'):
    ...
    for _ in range(patch_n//2):  #//2 1/2for half original kept, for data augmentation
        ...
        if augment_mode == 'advanced':
            patch_input_img = advanced_data_augmentation(patch_input_img, tmp)
            patch_target_img = advanced_data_augmentation(patch_target_img, tmp)
        else:
...
def get_loader(mode='train', load_mode=0,
               saved_path=None, test_patient='L506',
               patch_n=None, patch_size=None,
               transform=None, batch_size=32, shuffle=True, num_workers=6, augment_mode='basic'):
    dataset_ = ct_dataset(mode, load_mode, saved_path, test_patient, patch_n, patch_size, transform, augment_mode=augment_mode)
