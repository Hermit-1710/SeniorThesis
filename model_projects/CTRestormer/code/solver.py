...
class TeeLogger:
    ...
    def write(self, message):
        print(message)
        if self.log_path and not self.log_disabled:
            try:
                with open(self.log_path, 'a', encoding='utf-8') as f:
                    f.write(message + '\n')
            except PermissionError:
                self.log_disabled = True
                print('Warning: cannot write log file {}, continuing without file logging.'.format(self.log_path))
...
def load_loss_history(save_path, resume_iters):
    if resume_iters <= 0:
        return []
    loss_path = os.path.join(save_path, 'loss_{}_iter.npy'.format(resume_iters))
    if not os.path.exists(loss_path):
        return []
    return np.load(loss_path).astype(np.float32).tolist()
...
class Solver(object):
    ...
    def build_model(self, for_test=False):
        if self.model_name == 'ctrestormer':
            from ctrestormer import CTRestormer
            return CTRestormer(
                inp_channels=1,
                out_channels=1,
                dim=self.restormer_dim,
                num_blocks=tuple(self.restormer_num_blocks),
                num_refinement_blocks=self.restormer_num_refinement_blocks,
                heads=tuple(self.restormer_heads),
                ffn_expansion_factor=self.restormer_ffn_expansion_factor
            )
        raise ValueError('This project only supports model_name=ctrestormer, got {}'.format(self.model_name))
    ...
    def train(self):
        ...
        train_losses = load_loss_history(self.save_path, self.resume_iters)
        total_iters = self.resume_iters
        start_time = time.time()
        loss_all = list(train_losses)
        if train_losses:
            self.logger.write('loaded loss history: {} values'.format(len(train_losses)))
        stop_training = False
